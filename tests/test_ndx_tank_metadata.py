...
class LabMetaDataExtensionTest(unittest.TestCase):
    ...
    def test_add_lab_metadata(self):
        ...
        maze_extension = MazeExtension(
            name="mazes", description="description of the mazes"
        )
        mazes_dict = {k: "1" for k in maze_extension.mazes_attr}
            ...
            for metadata_key, metadata_value in lab_metadata_dict.items():
                if isinstance(metadata_value, RigExtension):
                    ...
                    for mazes_key, mazes_value in mazes_dict.items():
                        assert (
                            mazes_value in getattr(metadata_value, mazes_key, None).data
                        )
                else:
                    self.assertEqual(
                        metadata_value,
                        getattr(
                            nwbfile.lab_meta_data["LabMetaData"], metadata_key, None
                        ),
                    )
